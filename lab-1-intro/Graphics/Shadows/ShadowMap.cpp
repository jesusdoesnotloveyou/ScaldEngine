	...
	mWidth(width), mHeight(height), mDepthMapSRV(nullptr), mDepthMapDSV(nullptr)
{
	...
	ThrowIfFailed(device->CreateTexture2D(&texDesc, 0, &depthMap));
	...
	ThrowIfFailed(device->CreateDepthStencilView(depthMap, &dsvDesc, &mDepthMapDSV));
	...
	ThrowIfFailed(device->CreateShaderResourceView(depthMap, &srvDesc, &mDepthMapSRV));
