...
public:
    ...
    virtual RetVal Execute(Args&&... args) override { return Execute_Internal(std::forward<Args>(args)..., std::index_sequence_for<Args2...>()); }

    virtual void Clone(void* pDestination) override { new (pDestination) StaticDelegate(m_Function, m_Payload); }
...
public:
    ...
    virtual void Clone(void* pDestination) override { new (pDestination) RawDelegate(m_pObject, m_Function, m_Payload); }
...
public:
    ...
    RetVal Execute(Args&&... args) override { return Execute_Internal(std::forward<Args>(args)..., std::index_sequence_for<Args2...>()); }

    virtual void Clone(void* pDestination) override { new (pDestination) LambdaDelegate(m_Lambda, m_Payload); }
...
public:
    ...
    virtual const void* GetOwner() const override { return m_pObject.expired() ? nullptr : m_pObject.lock().get(); }

    virtual void Clone(void* pDestination) override { new (pDestination) SPDelegate(m_pObject, m_pFunction, m_Payload); }
