...
def generate_project_files():
    if not os.path.exists(Config.BUILD_FOLDER):
        ...
        print(f'Create {Config.BUILD_FOLDER} folder')
...
def build_project(configuration=Configuration.Release):
    ...
    if run_command(command):
        print(f'Project has been built successfully in {configuration} mode.')
    else:
        print(f'Failed to build the project {configuration} mode.')
    os.chdir("..")
...
def run_clang_format(source_dir):
    ...
    if not format_sources:
        print(f'No source files founded in {source_dir}.')
        return
